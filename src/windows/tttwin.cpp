...
bool WinCheck(vector<vector<string>> grid,string symbol){
	
	vector<string> testCases(grid.size());
	
	string checkstrh,checkstrv,checkstrlr,checkstrrl,exp=string(grid.size(),char(symbol[0]))+"_";
...
int main(){
    ...
    cout<<"To win atleast a row or a column or a diagonal \nmust be filled with the same character.\n\n";
	...
	while(1){
		for(int p=0;p<players.size();p++){
			...
			DrawPoints(players);
			DrawGrid(grid);
			if(WinCheck(grid,players[p].getSymbol())){
			...
			if(WinCheck(grid,players[p].getSymbol())==false and filled(grid)==true){
				system("cls");
				DrawPoints(players);
				DrawGrid(grid);
				cout<<"It's a draw!\n";
